    ...
    with progress:
        for class_name, class_data in data_frames.items():
            ...
            for df in class_data:
                ...
                logger.debug(f"Data from class '{class_name}' has {len(cur_data)} rows")
                progress.update(prepare_barid, 1)
...
def convert_mapped_data_to_dataframes(
    mapped_data: Dict[str, List], target_schema: SchemaView
):
        ...
        if max_processes == 1:
            ...
            results = [
                pool.apply_async(
                    ...
                    (),
                    map_arg,
                    callback=lambda x: run_mapper_progress.update(map_barid, 1),
                )
